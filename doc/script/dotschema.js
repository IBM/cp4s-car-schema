...
class Node {
  ...
  toString() {
    return `"${this.id}" [id="${this.id}", type="${this.type}", shape="${this.shape}", label="${
      this.objectType
    } | {${this.id} | ${this.attributesToString()}}}"];\n`;
  }
}
...
class DotSchema {
  ...
  loadCoreSchema() {
    ...
    const edgesSubGraph = new Graph({ label: 'Edges' }, graphTypes.SUBGRAPH, layouts.DETAIL_LAYOUT);
    ...
    for (const collectionName in coreSchema.edges) {
      ...
      edgesSubGraph.nodes.push(
        this.createNode(collectionName, coreSchema.edges[collectionName], objectTypes.EDGE)
      );
    }
  }
  ...
  createEdge(collectionName, collectionSchema) {
    ...
    else return [new Edge(collectionSchema.source, collectionSchema.targets[0])];
  }
